						...
						{i < STAGES.length - 1 && (
							<div
								style={{
									...
									height: '1px',
									background:
										s.id < current ? '#c4923a' : 'rgba(196,146,58,0.15)',
									marginBottom: '22px',
									flexShrink: 0
								}}
							/>
		...
		<div>
			...
			>
				{['signin', 'signup'].map((m) => (
					...
					>
						{m === 'signin' ? 'Logowanie' : 'Rejestracja'}
					</button>
				))}
			</div>
		...
		<div>
			...
			>
				...
				<div
					style={{
						gridColumn: '1 / -1',
						borderTop: '1px solid rgba(196,146,58,0.1)',
						paddingTop: '20px'
					}}
				>
					<p
...
function Step3({ selectedBerth, selectedSegment, onNext, onBack }) {
	const seg =
		VOYAGE_SEGMENTS.find((s) => s.id === selectedSegment) || VOYAGE_SEGMENTS[0]
	...
	return (
		<div>
			...
			>
				...
				<div
					style={{ padding: '14px 24px', background: 'rgba(196,146,58,0.05)' }}
				>
					<p
						style={{
							fontFamily: 'DM Sans, sans-serif',
	...
	return (
		<div>
				...
				>
					...
					>
						...
						>
							...
							>
								{seg.price.toLocaleString('pl-PL')} zł
							</span>
						</div>
						<div
							style={{
			...
			<div style={{ display: 'flex', gap: '16px', marginTop: '32px' }}>
				...
				>
					Zapłać {seg.price.toLocaleString('pl-PL')} zł →
				</button>
			</div>
		</div>
	)
...
function Step5({ selectedBerth, selectedSegment, onDashboard }) {
	...
	return (
		...
		>
			...
			>
				Koja{' '}
				<strong style={{ color: '#f5f0e8' }}>{selectedBerth || 'A1'}</strong> na
				etapie <strong style={{ color: '#f5f0e8' }}>{seg.name}</strong> (
				{seg.dates}) jest Twoja. Potwierdzenie zostało wysłane e-mailem.
			</p>
