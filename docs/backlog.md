...
- **SEC-3 — Backup produkcyjnego deploymentu Convex.** Dashboard prod pokazuje „No backup yet" (07-13). Baza z rezerwacjami/płatnościami bez backupu. Włączyć w Deployment Settings → Backup & Restore; sprawdzić dostępność/koszt na darmowym planie (może wymagać Pro). Do czasu backupów automatycznych: okresowy `npx convex export` ręcznie. *(wykryte 07-13 przy weryfikacji regionu)*
