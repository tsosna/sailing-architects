...
- **SEC-3 — Backup produkcyjnego deploymentu Convex.** ~~„No backup yet"~~ pierwszy backup ręczny zrobiony + pobrany 07-13 (Tomek). Zostaje: (a) **automatyzacja** — harmonogram backupów w Backup & Restore (sprawdzić dostępność na darmowym planie, może wymagać Pro) albo cykliczny `npx convex export`; (b) **zasady dla pobranych eksportów** (zawierają dane osobowe plaintext): poza Downloads i poza repo, dysk z FileVault albo zip z hasłem, kasować stary przy nowym (retencja). *(wykryte 07-13 przy weryfikacji regionu)*
