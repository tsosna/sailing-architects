...
- **SEC-2 — Region EU Convex: decyzja migracji + komplet DPA.** (a) ~~Sprawdzić czy Convex oferuje region EU~~ ✅ 07-13: **TAK — Europe (Ireland)**, zweryfikowane w dashboardzie (Team Settings → Default Region; Tomek przełączył default na Europe). ALE: przełącznik dotyczy tylko **nowych** deploymentów — istniejący dev+prod zostają w US East; „dane w Europie" wymaga migracji (nowy deployment EU: eksport/import danych, nowy `PUBLIC_CONVEX_URL` w Vercel, nowe deploy keys, env vars od nowa, rekonfiguracja Clerk JWT, smoke — rozmiar ~A6; im wcześniej tym mniej danych). Koszt EU: **+30% regional surcharge, no included limits (billing on-demand od zera)** — decyzja finansowa Michała: macierz US+SEC-1 („dokumenty szyfrowane") vs migracja EU („dane w Europie") vs oba. SEC-1 robić niezależnie (region nie chroni przed wyciekiem dumpa). (b) DPA od Convex, Clerk, Stripe, Brevo (wiąże się z LEGAL-2); transfer do USA legalny przy DPF/SCC; obowiązek zgłoszenia naruszenia 72h istnieje zawsze. *(dyskusja Tomek 07-13)*
